...
void main() {
    ...
    vec3 waterEdge = vec3(0.55, 0.65, 0.50);
    ...
    if (heightNorm < 0.02) {
        ...
        texColor = mix(grassColor, darkGrass, smoothstep(0.08, 0.3, heightNorm));
    } else if (heightNorm < 0.55) {
        texColor = mix(darkGrass, rockColor, smoothstep(0.3, 0.55, heightNorm));
    ...
    if (slope > 0.35) {
        texColor = mix(texColor, rockColor, smoothstep(0.35, 0.6, slope));
    }
